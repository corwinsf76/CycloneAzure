...
# Use absolute import to avoid ImportError
from dashboard import data_provider # To get initial dropdown options
# from .. import config
...
def create_market_chart() -> html.Div:
    """Create the market data chart layout."""
    return html.Div([
        dcc.Graph(
            id='market-chart',
            style={'height': '60vh'}
...
def create_sentiment_chart() -> html.Div:
    """Create the sentiment analysis chart layout."""
    return html.Div([
        dcc.Graph(
            id='sentiment-chart',
            style={'height': '40vh'}
...
def create_pnl_chart() -> html.Div:
    """Create the PnL tracking chart layout."""
    return html.Div([
        dcc.Graph(
            id='pnl-chart',
            style={'height': '40vh'}
...
def create_layout() -> html.Div:
    ...
    return html.Div([
        ...
        create_market_chart(),
        html.Div([
            html.Div([create_sentiment_chart()], style={'flex': 1}),
            html.Div([create_pnl_chart()], style={'flex': 1})
    ...
    return dbc.Container([
        dbc.Row([
            dbc.Col(dcc.Dropdown(
                ...
                options=[{'label': option['label'], 'value': option['value']} for option in initial_symbol_options],
...
def create_market_insights_tab():
    """Creates a tab showing insights from all data sources."""
    return dbc.Tab(
        label="Market Insights",
        children=[
            dbc.Row([
                # CryptoPanic Sentiment Card
                dbc.Col([
                    dbc.Card([
                        dbc.CardHeader("Market Sentiment (CryptoPanic)"),
                        dbc.CardBody([
                            dcc.Graph(id='cryptopanic-sentiment-graph'),
                            html.Div(id='cryptopanic-stats')
                        ])
                    ])
                ], width=6),
                
                # AlphaVantage Health Card
                dbc.Col([
                    dbc.Card([
                        dbc.CardHeader("Market Health (AlphaVantage)"),
                        dbc.CardBody([
                            dcc.Graph(id='alphavantage-health-graph'),
                            html.Div(id='alphavantage-stats')
                        ])
                    ])
                ], width=6)
            ], className='mb-4'),
            ...
            dbc.Row([
                # CoinGecko Metrics Card
                dbc.Col([
                    dbc.Card([
                        dbc.CardHeader("Market Metrics (CoinGecko)"),
                        dbc.CardBody([
                            dcc.Graph(id='coingecko-metrics-graph'),
                            html.Div(id='coingecko-stats')
                        ])
                    ])
                ], width=12)
            ])
